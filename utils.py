...
#             reader=PILReader,
#         ),
#         EnsureChannelFirstd(keys=["image"], channel_dim=-1),  # For RGB images
#         EnsureChannelFirstd(keys=["label"], channel_dim="no_channel"),  # Replace AddChanneld
#         ScaleIntensityd(keys=["image"]),
#         EnsureTyped(keys=["image", "label"], device=device, track_meta=False),
#         # RandCropByPosNegLabeld(
#         #     keys=["image", "label"],
#         #     label_key="label",
#         #     spatial_size=(args.roi_size, args.roi_size),
#         #     pos=1,
#         #     neg=1,
#         #     num_samples=1,
#         # ),
...
#     )
#     # train_loader = ThreadDataLoader(train_ds, num_workers=0, batch_size=args.b, shuffle=True)
#     train_loader = DataLoader(train_ds, num_workers=0, batch_size=args.b, shuffle=True)
...
def generate_click_prompt(img, msk, pt_label=1):
    # Return: image, prompt points, prompt masks
    pt_list = []
    msk_list = []
    b, c, h, w = msk.size()
    msk = msk[:, 0, :, :]  # Assuming masks have one channel
    for j in range(b):
        msk_s = msk[j, :, :]  # Shape: [h, w]
        indices = torch.nonzero(msk_s)
        if indices.size(0) == 0:
            # Generate random indices between [0, h-1] and [0, w-1]
            y = torch.randint(0, h, (1,)).item()
            x = torch.randint(0, w, (1,)).item()
            random_index = torch.tensor([y, x], device=msk.device)
            new_s = msk_s
        else:
            random_idx = torch.randint(0, indices.size(0), (1,)).item()
            random_index = indices[random_idx]  # Shape: [2]
            y, x = random_index[0], random_index[1]
            label = msk_s[y, x]
            new_s = torch.zeros_like(msk_s)
            # Create a mask where the selected label is 1, others are 0
            new_s = (msk_s == label).float()
        pt_list.append(random_index)
        msk_list.append(new_s)
    pt = torch.stack(pt_list, dim=0)  # Shape: [batch_size, 2]
    msk = torch.stack(msk_list, dim=0)  # Shape: [batch_size, height, width]
    msk = msk.unsqueeze(1)  # Add channel dimension: [batch_size, 1, height, width]
    return img, pt, msk
...
def generate_click_prompt(img, msk, pt_label=1):
    ...
    if len(size) == 5:
        # 3D data
        b, c, h, w, d = size
        msk = msk[:, 0, :, :, :]  # Assuming single-channel masks
    elif len(size) == 4:
        # 2D data
        b, c, h, w = size
        d = 1  # Depth is 1 for 2D data
        msk = msk[:, 0, :, :]  # Shape: [b, h, w]
    else:
        raise ValueError(f"Unsupported tensor size: {size}")
    ...
    for j in range(b):
        ...
        if indices.size(0) == 0:
            ...
            if d == 1:
                random_index = [
                    torch.randint(0, h, (1,)).item(),
                    torch.randint(0, w, (1,)).item()
                ]
            else:
                random_index = [
                    torch.randint(0, h, (1,)).item(),
                    torch.randint(0, w, (1,)).item(),
                    torch.randint(0, d, (1,)).item()
                ]
            new_s = msk_s.clone()
        else:
            # Select a random index from the nonzero indices
            idx = torch.randint(0, indices.size(0), (1,)).item()
            random_index = indices[idx].tolist()
            new_s = torch.zeros_like(msk_s)
            if d == 1:
                label = msk_s[random_index[0], random_index[1]]
            else:
                label = msk_s[random_index[0], random_index[1], random_index[2]]
            new_s = (msk_s == label).float()
        
        pt_list.append(random_index)
        msk_list.append(new_s)
    
    # Convert point list to tensor
    pt = torch.tensor(pt_list, dtype=torch.float, device=msk.device)  # Shape: [b, 2] or [b, 3]
    
    # Stack masks into a tensor
    if d == 1:
        msk = torch.stack(msk_list, dim=0)  # Shape: [b, h, w]
        msk = msk.unsqueeze(1)  # Shape: [b, 1, h, w]
    else:
        msk = torch.stack(msk_list, dim=0)  # Shape: [b, h, w, d]
